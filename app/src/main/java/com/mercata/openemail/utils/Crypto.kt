...
import com.mercata.openemail.NONCE_HEADER_ALGORITHM_KEY
import com.mercata.openemail.NONCE_HEADER_PUBKEY_KEY
import com.mercata.openemail.NONCE_HEADER_SIGNATURE_KEY
import com.mercata.openemail.NONCE_HEADER_VALUE_HOST
import com.mercata.openemail.NONCE_HEADER_VALUE_KEY
import com.mercata.openemail.NONCE_SCHEME
import com.mercata.openemail.SIGNING_ALGORITHM
import com.mercata.openemail.headerFieldSeparator
import com.mercata.openemail.headerKeyValueSeparator
...
@Throws(SodiumException::class)
fun encryptAnonymous(data: ByteArray, publicEncryptionKey: ByteArray): ByteArray {
    val cipher = ByteArray(Box.SEALBYTES + data.size)

    if (!sodium.cryptoBoxSeal(cipher, data, data.size.toLong(), publicEncryptionKey)) {
